...
public class AuthenticationActivity extends AppCompatActivity {
    ...
    public void onCreate (Bundle savedInstanceState) {
        ...
        if (currentUser!=null) {changeActivity(InformationActivity.class,false);}
        signInOrRegister.setOnClickListener((View v) -> {
            loadingProgressBar.setVisibility(View.VISIBLE);
            String email = emailEditText.getText().toString();
            String password = passwordEditText.getText().toString();
            auth.signIn_signUp_WithEmailAndPassword(email, password, new AuthenticationController.AuthListener() {
                @Override
   ...
   private <Activity>  void changeActivity  (Class<Activity> activity,boolean addBackStack){
       Intent intent =
